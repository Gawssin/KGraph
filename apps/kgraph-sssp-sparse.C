...
void pivotSSP(graph<vertex> &GB, int pivotId, std::vector<WN> &pivotSP, int *pivotDist,
              std::priority_queue<WNH, std::vector<WNH>, std::greater<WNH>> &pq, int *vis)
{
...
void Compute(graph<vertex> &GA, graph<vertex> &GB, int NP, std::vector<int> &pivot_l, commandLine P)
{
  ...
  parallel_for(int i = 0; i < k; i++)
  {
    ...
    lq_l[i][pid].push(std::make_pair(ShortestPathLen_l[i][s_l[i]], s_l[i]));
    closestPart[i][pid] = 0;
  }
        ...
        if (td == ShortestPathLen[vi])
        {
          ...
          for (int d = 0; d < degree; d++)
          {
            ...
            if (ShortestPathLen[ngh] > ShortestPathLen[vi] + v.getOutWeight(d))
            {
              ...
              local_frontier[nghPart].push(std::make_pair(ShortestPathLen[ngh], ngh));
          ...
          if (id2pivotIndex[vi] != -1)
          {
            ...
            for (int j = 0; j < pivotSP[pivotIndex].size(); ++j)
            {
              ...
              if (ShortestPathLen[ngh] > ShortestPathLen[vi] + wgh)
              {
                ...
                pq.push(std::make_pair(ShortestPathLen[ngh], ngh));
          ...
          else
          {
            ...
            for (int d = 0; d < degreeb; ++d)
            {
              ...
              if (ShortestPathLen[ngb] > ShortestPathLen[vi] + vb.getOutWeight(d))
              {
                ...
                pq.push(std::make_pair(ShortestPathLen[ngb], ngb));
